...
young_modulus_body = rospy.get_param('YoungModulusBody', 170e6)  # (Pa)
young_modulus_tip = rospy.get_param('YoungModulusTip', 21e6)     # (Pa)
length_body = rospy.get_param('LengthBody', 0.5)                 # (m)
length_tip = rospy.get_param('LengthFlexibleTip', 0.034)         # (m)
outer_diam = rospy.get_param('OuterDiam', 0.00133)               # (m)
inner_diam = rospy.get_param('InnerDiam', 0.0008)                # (m)
...
# Parameter magnet
magnet_length = rospy.get_param('Lm', 4e-3)           # (m)
magnet_id = rospy.get_param('dim', 0.86e-3)           # (m)
magnet_od = rospy.get_param('dom', 1.33e-3)           # (m)
magnet_remanence = rospy.get_param('Br', 1.45)        # (T)

# Parameter nodes and elements
nume_nodes_viz = 600
num_elem_body = 30
num_elem_tip = 3
...
def createScene(root_node):
    ...
    controller_ros = mcr_controller_ros.ControllerRos(
        root_node=root_node,
        e_mns=navion,
    ...
    root_node.addObject(controller_ros)
